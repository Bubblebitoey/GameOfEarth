...
public class Creator {
	...
	private void makeList() {
		...
		addQuestion("Print Money", "Our government loans are getting bigger every year. Cut a forest to print lots of money so we can pay off the debt?", 320, 0, -280, 0);
		addQuestion("Parenting License", "Parenting requires real knowledge. To act on rising child abandonment, we must make all parents pay for a license before having a child?", 0, -25, 0, 30);
		addQuestion("Post-Disaster", "Our airport was destroyed by the tornado. Repair the damaged airport?", 200, 6, 0, -8);
		addQuestion("Monsoon Season", "Weather guy predicts a monsoon to hit our annual sports event. Let's clear the clouds so we can attract more tourists for the celebration!", 480, 18, -400, -20);
		addQuestion("Free housing", "Rising property prices force our newly couples to live with their parents, build more free housing for our citizens?", 400, 23, -300, -30);
		addQuestion("Car Emissions Coverup", "A car company miscalculated their cars carbon emissions. Let's keep quite?..I mean its been there for years now and nobody noticed.", 321, 0, -410, 1);
		addQuestion("More street lamps", "The new 'Slapamon Go' game is causing people to ealk into ponds. Build more street lamps?", 600, 30, -590, -40);
		addQuestion("New Wind Farms", "The winds have changed. Our wind farms are becoming ineffective and we end up burning coal to meet demands. Build new wind farms?", -300, 0, 220, 0);
		addQuestion("Exercise For everyone", "Obesity related diseases are on the rise. Obesity doesn't run in the family, the problem is no one runs in the family. Mandatory exercise for everyone", 0, 18, 0, -20);
		addQuestion("Recycle trucks", "Citizens are only recycling on the one day of the week that recycle truck comes. Have them come every day instead?", -6000, 34, 400, -29);
		addQuestion("Use Dying Reef", "Land prices are so high, citizens are migrating overseas. Reclaim land over the coral reef and sell the land to developers?", 400, 46, 120, -18);
		addQuestion("Find the lazy", "We should put trackers on everyone in the country to see if anyone is being lazy.. To enhance productivity, of course.", 350, -10, -400, 0);
		addQuestion("Remove Air Pollutants", "With new city construction, mass amounts of pollutants are floating in the air. All buildings should have an air scrubber to help remove these pollutants.", -400, 10, 270, -28);
		addQuestion("Cheap Public Transport", "There are always traffic jams during peaks hours. Let's smooth out the traffic with cheap public transport.", -300, 0, 400, 0);
		addQuestion("Cryo-Freeze the Sick", "Allow unprecedented court ruling to cryo-freeze the little girl until the day we can cure her cancer?", 200, 48, -300, -39);
		addQuestion("Coverup Deforestation", "A state-owned logging company has been caught with illegal deforestation, let's cover this up by releasing fake reports.", 200, 0, -230, 0);
		addQuestion("Drug Addiction Hotline", "Rising number of students are turning to drugs for stress relief. Setup hotlines to help students with their problems?", 0, 16, 0, -23);
		addQuestion("Local Farmers", "Imported vegetables are expensive. Subsidize local farmers for producing cheap local veggies?", -240, 0, 250, 0);
		addQuestion("Remove Speed Bumps", "Constant speeding up and slowing down gives off more emissions. Remove all speed bumps?", -340, -23, 200, 20);
		addQuestion("Free money", "Financial problems are the top reason for divorce. Give out free money to couples to keep them together?", 500, 39, -400, 0);
		addQuestion("Child labor", "Child labor laws creating homeless starving children. Check the factory environment and allow the children to work for a warm meal?", 300, 21, -200, 0);
		addQuestion("Under 21 sales Tax", "Spoiled kids are overspending their parents fortune. Add a sale tax for spenders under 21?", -400, -21, 380, 24);
		...
		addQuestion("Plastic bottle Refunds", "A lot of the nondegradable plastic is ending up in our landfills. Install refund machines so that every 20,000 plastic bottles collected could return a plane ticket.", -100, 10, 120, -11);
		addQuestion("Breastfeeding", "Breastfeeding is beneficial to both the baby and mother. Sadly new mothers resort to milk powder for convenient. Subsidize breast feeding?", -400, 8, 0, -11);
		addQuestion("Prevent illegal Mining", "Citizens have been illegally digging up coal in their backyards. Deport them from their home to prevent this?", -300, -4, 320, 3);
